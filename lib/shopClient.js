...
/**
 * Rough delivery-cost estimate for a postcode. Never cached. Returns
 * { available: false, note } if the store can't quote right now.
 */
async function getShippingQuote({ postcode, suburb, productSlug, quantity } = {}) {
  return request("/api/bot/shipping", {
    method: "POST",
    body: {
      postcode,
