...
disc brakes, speed modes a parent can limit, arrive ~80% pre-assembled with a
charger and a basic toolkit in the box.
...
- Side-by-side comparison: gobike.au/bikes#compare-models.
...
## Payments & checkout
- GoBike sells within Australia only, prices in AUD.
...
- Classed in Australia as low-powered power-assisted pedal cycles - legal for
  kids to ride on private property. Public road and path rules vary by state
  and territory, so tell the customer to check their state's rules rather than
  giving a blanket yes or no (NSW example: transport.nsw.gov.au e-bike page).
