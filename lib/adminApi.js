...
const { selfTest } = require("./claudeAgent");
  ...
  wrap(async (req, res) => {
    ...
    res.json({
      ...
      config: {
        ...
        telegram_notify: notify.configured(),
      },
      webhook_url:
...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    await db.clearThreadMemory({ platform, senderId });
    res.json({ ok: true });
  })
);
...
// Live end-to-end check: does the Claude API actually work with the current
// key + model, and is the store reachable? Surfaces the exact error.
router.get(
  "/selftest",
  wrap(async (req, res) => {
    const [claude, catalog] = await Promise.all([
      selfTest(),
      shopClient
        .getCatalog({ force: true })
        .then((c) => ({ ok: Boolean(c), products: c ? c.productCount : 0 }))
        .catch((e) => ({ ok: false, error: e.message })),
    ]);
    res.json({ claude, store: catalog });
  })
);
...
// Fire a test message to the Telegram group so the owner can confirm setup.
router.post(
  "/notify-test",
  wrap(async (req, res) => {
    if (!notify.configured()) {
      return res.json({
        ok: false,
        error:
          "TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID aren't set on the server yet.",
      });
    }
    const result = await notify.sendTestNotification();
    res.json(result.ok ? { ok: true } : { ok: false, error: result.body || result.error || "send failed" });
  })
);
