 ...
 */
async function flagForHuman(ctx, reason, { alwaysNotify = false } = {}) {
  ...
  // Ping the team (Telegram) once per fresh handoff — or every time when the
  // caller says so (an explicit "I want a human" shouldn't be swallowed by the
  // handoff dedup; the caller throttles how often that can fire).
  if (isNew || alwaysNotify) {
...
module.exports = { generateReply, selfTest, flagForHuman };
