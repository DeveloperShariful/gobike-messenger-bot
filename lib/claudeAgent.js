 ...
 */
async function generateReply({ platform, senderId, userText, history, adContext }) {
  ...
  // If the customer arrived from an ad, give the model that context on this
  // turn only (kept out of the cached system prompt so per-customer context
  // doesn't fragment the cache). It's a note, not the customer's words.
  let currentUserContent = userText;
  if (adContext) {
    currentUserContent =
      `[Context, not from the customer: they arrived by clicking a GoBike ad` +
